...
*Note: This blog entry is an update of a [previous article](../../../2022/01/install-jaxon-adminer-on-voyager.html) written about how to install the `Jaxon DbAdmin` package on `Voyager`.*
...
As an additional consequence, the authentication and security features are not implemented in the package, which will then rely on those provided by the application it is installed on.
